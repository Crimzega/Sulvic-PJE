...
public class EnumHelper{
	...
	private static <T> boolean isCommonType(Class<T> enumType){
		for(Class<?>[] lookup: commonTypes) if(lookup[0] == enumType) return true;
		return false;
	}
	
	/* Everything below this is found at the site below, and updated to be able to compile in Eclipse/Java 1.6+ Also modified for
	 * use in decompiled code. Found at: http://niceideas.ch/roller2/badtrash/entry/java_create_enum_instances_dynamically */
	...
	private static <T> void addCommonType(Class<T> enumType, Class<?>[] enumTypes){
		Class<?>[] newCommonType = new Class<?>[enumTypes.length + 1];
		newCommonType[0] = enumType;
		for(int i = 0; i < enumTypes.length; i++) newCommonType[i + 1] = enumTypes[i];
		Class<?>[][] temp = new Class<?>[commonTypes.length + 1][];
		for(int i = 0; i < commonTypes.length; i++) temp[i] = commonTypes[i];
		temp[commonTypes.length] = newCommonType;
		commonTypes = temp;
	}
	...
	public static <T extends Enum<?>> T addEnum(Class<T> enumType, String enumName, Object... paramValues){
		...
		if(isCommonType(enumType)){
			...
			addCommonType(enumType, paramTypes);
